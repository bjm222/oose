$(document).ready(function() {
 ...
 console.log(localStorage.endDate);
    ...
    $.ajax({
        url: API +'announcement/manage/bydate/'+localStorage.startDate+'/'+localStorage.endDate, 
        async: false,
    ...
    <h4>' + item.pushDate + '</h4><div style="line-height:60%;"><br></div>';
        if(item.isEvent == true) {
            ...
            if(item.location == null) {
                ...
                showDetails += '<h4>Lat: '+item.locationLat.toFixed(2)+" "+
                '<span>Lng: </span>'+item.locationLng.toFixed(2)+'</h4>';
            } else {
